...
func TestRepositoryStructuredMigrationCatalogLoads(t *testing.T) {
	...
	for _, token := range []string{"enable_plugin_was_null", "DROP TABLE IF EXISTS conversation_policy_snapshot_backups", "DROP COLUMN IF EXISTS pinned_at", "DROP COLUMN IF EXISTS free_auto_select_priority", "DROP COLUMN IF EXISTS free_auto_select_base_urls", "ON public.skills(owner_user_id, category, skill_name)", "AS identity_conflicts", "AS relative_root_conflicts", "DROP TABLE IF EXISTS public.conversation_workspace_bindings", "DROP TABLE IF EXISTS public.local_workspaces"} {
