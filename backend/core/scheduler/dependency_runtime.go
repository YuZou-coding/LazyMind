...
func taskOutputBody(result string) string { return taskcenter.OutputBody(result) }

type artifactManifestItem = taskcenter.OutputArtifact

func finalizeTaskOutput(ctx context.Context, db *gorm.DB, taskID, convID string) string {
	status, _ := taskcenter.FinalizeTaskOutput(ctx, db, taskID, convID)
	return status
}
