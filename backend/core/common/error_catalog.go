...
var errorCatalog = map[string]*AppError{
	"notification configuration invalid":  NewAppError(http.StatusBadRequest, ErrCodeNotificationInvalid, "通知配置无效"),
	"notification version conflict":       NewAppError(http.StatusConflict, ErrCodeNotificationConflict, "通知配置已更新，请刷新后重试"),
	"notification provider unsupported":   NewAppError(http.StatusBadRequest, ErrCodeNotificationUnsupported, "该渠道暂不支持任务通知"),
	"notification disable impact changed": NewAppError(http.StatusConflict, ErrCodeNotificationImpact, "通知关闭影响已变化，请重新确认"),
	"notification duplicate risk":         NewAppError(http.StatusConflict, ErrCodeNotificationDuplicate, "发送结果未知，重试可能重复，请确认后继续"),
	"notification dependency unavailable": NewAppError(http.StatusServiceUnavailable, ErrCodeNotificationUnavailable, "暂时无法验证通知目标，请稍后重试"),
